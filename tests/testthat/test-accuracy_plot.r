...
test_that("`accuracy_plot` should return a ggplot with x and y axis properly labeled", {
    expect_equivalent(accuracy_plot(tibble_2_row)$labels$x, "Neighbors")
    expect_equivalent(accuracy_plot(tibble_2_row)$labels$y, "Accuracy Estimate")
})

test_that("`accuracy_plot` should return a ggplot and plot layers match expectations - Testing within the layers", {
    expect_equivalent(accuracy_plot(tibble_2_row)$data$mean, accuracy_tibble_2_row$data$mean)
    expect_equivalent(accuracy_plot(tibble_2_row)$data$neighbors, accuracy_tibble_2_row$data$neighbors)
    expect_equivalent(accuracy_plot(tibble_5_row)$data$mean, accuracy_tibble_5_row$data$mean)
    expect_equivalent(accuracy_plot(tibble_5_row)$data$neighbors, accuracy_tibble_5_row$data$neighbors)
})
