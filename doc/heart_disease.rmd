...
heart_disease <- read.csv("../data/processed/full.csv", header = FALSE)
knitr::kable(head(heart_disease), caption = "Heart Disease Dataset")
``` 


Before any data analysis can be done, the data needs to be cleaned. The downloaded data does not have any column names so first we need to add all the variables to their respective column. The next step in cleaning up the data is to check for NAs. It is important that we remove these before any data analysis so that the functions we use will run on the data properly. Instead of R recognizing our missing values, we have a question marks. We need to convert this to an actual NA that R will recognize. Thus to clean the data we have used our custom clean function. Additioanlly since this project should be in the scope of DSCI 100 and we did not learn how to use one hot encoding to deal with categorical variables, we will drop all the categorical variables from our dataset.
...
knitr::kable(head(heart_disease_numerical), caption = "Clean Heart Disease Dataset")
```
...
````{r attribute box, echo=FALSE, fig.dim = c(8, 6), fig.cap="Age vs Heart Disease Diagnosis."}
...
```{r}
diagnosis_heart_disease <- balance_check(heart_disease_clean, diagnosis)
diagnosis_heart_disease
```


```{r pie distribution, echo=FALSE, fig.dim = c(8, 6), fig.cap="Figure 3: Distribution of Diagnosis"}
...
```{r correlation, echo=FALSE, fig.dim = c(8, 6), fig.cap="Figure 4: Correlation between Variables"}
knitr::include_graphics("../results/varaible_correlation.png")
```
...
hd_data_split <- initial_split(heart_disease_numerical, prop = 0.75, strata = diagnosis)
data_train <- training(hd_data_split)
data_test <- testing(hd_data_split) 

train_counts <- summarise(training_data,
                          `Data partition` = "Training",
                          `Heart Disease Not Detected` = sum(diagnosis  == 0),
                          `Heart Disease Detected` = sum(diagnosis  == 1))
test_counts <- summarise(test_data,
                         `Data partition` = "Test",
                         `Heart Disease Not Detected` = sum(diagnosis  == 0),
                         `Heart Disease Detected` = sum(diagnosis  == 1))
kable(bind_rows(train_counts, test_counts),
      caption = "Counts of observation for each class for each data partition.")
```

As we can see in REFENECE FIGURE, each class is fairy balanced.
...
```{r accuracy plot, echo=FALSE, fig.dim = c(8, 6), fig.cap="Accuracy of K-NN Model with Neighbors"}
...
```{r confusion matrix, echo=FALSE, fig.dim = c(8, 6), fig.cap="Figure 6: Confusion Matrix of Results"}
